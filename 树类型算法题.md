...
需要学习以下几点操作：<br>
（1）返回值设置为全局变量，有时候因为递归函数、或是需要多个函数才能解决问题时，返回类型需要设置为全局变量。
（2）LinkedList的使用，大小不定，且经常插入删除结点（即使在表的末端），LinkedList更加适合。<br>
（3）**向上回溯时**不符合的结点从路径中删除的操作：把握住**向上回溯时**的时间节点，是在左右子树均递归完成/跳出递归函数时。
（4）不同的路径添加到集合中：不能添加同一变量path，否则后续修改会影响到已添加至集合的元素。使用new LinkedList<>(path)复制集合。
